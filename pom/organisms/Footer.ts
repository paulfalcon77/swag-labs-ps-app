...
export class Footer {
    ...
    async checkVisibleFooter(){
        ...
        await expect(this.footerMessage).toHaveText('© 2026 Sauce Labs. All Rights Reserved. Terms of Service | Privacy Policy')
    }

    async checkFooterLink() {
        await expect(this.socialTwitterButton).toHaveAttribute('href', 'https://twitter.com/saucelabs');
        await expect(this.socialFacebookButton).toHaveAttribute('href', 'https://www.facebook.com/saucelabs');
        await expect(this.socialLinkedInButton).toHaveAttribute('href', 'https://www.linkedin.com/company/sauce-labs/');
    }
